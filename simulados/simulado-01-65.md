# Simulado completo 01 — 65 questões
...
> **ID:** `SIM01-Q001` · **Domínio:** D4 · **Dificuldade:** fácil · **Tempo:** 90 s · **Pontos:** 1

A universidade saber precisa atender a uma auditoria sem redesenhar toda a aplicação. A aplicação é global e a latência percebida pelo usuário é uma métrica de negócio. Uma empresa possui muitas contas AWS e quer uma fatura consolidada, visão central de custos e melhor aproveitamento agregado de descontos elegíveis. Qual recurso usar? Escolha a alternativa que atende diretamente ao requisito.

- **A)** Compartilhar a senha do usuário raiz entre todas as equipes.
- **B)** Gerenciar as contas com AWS Organizations e consolidated billing, estruturando OUs, tags e controles de compartilhamento de descontos.
...
- **B:** Correta — Organizations consolida cobrança e permite que uso agregado e compartilhamento elegível de descontos melhorem aproveitamento, além de centralizar relatórios.
...
## 2. Acesso entre contas com IAM Roles

> **ID:** `SIM01-Q002` · **Domínio:** D1 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A agência pública portal aberto precisa corrigir uma descoberta da revisão Well-Architected. O orçamento é limitado e a operação manual deve ser mínima. Uma aplicação na conta de produção precisa ler objetos de um bucket pertencente à conta de dados, sem armazenar chaves de acesso. O acesso deve ser temporário, auditável e seguir privilégio mínimo. Qual solução atende melhor? Considere o principal trade-off operacional e escolha a melhor solução.

- **A)** Criar um usuário IAM na conta de dados e copiar access key e secret key para a instância.
- **B)** Tornar o bucket público e restringir o endereço IP na aplicação.
- **C)** Criar uma IAM Role na conta de dados, confiar na role da aplicação e usar AWS STS AssumeRole com política limitada ao bucket.
- **D)** Compartilhar a senha do usuário raiz da conta de dados pelo Secrets Manager.
...
- **A:** Incorreta — Chaves de longa duração aumentam o risco de vazamento e exigem rotação; não são necessárias para workloads na AWS.
- **B:** Incorreta — Exposição pública viola o requisito e endereço IP não substitui autenticação e autorização.
- **C:** Correta — AssumeRole entrega credenciais temporárias e separa a política de confiança da política de permissões, permitindo acesso entre contas sem segredo de longa duração.
- **D:** Incorreta — Credenciais do usuário raiz não devem ser usadas por aplicações, mesmo quando armazenadas em um cofre.
...
- **A:** Incorreta — Snapshots ajudam em restauração, mas não fornecem failover rápido nem RPO próximo de zero.
...
> **ID:** `SIM01-Q004` · **Domínio:** D3 · **Dificuldade:** fácil · **Tempo:** 90 s · **Pontos:** 1

A plataforma de mídia onda precisa atender a uma auditoria sem redesenhar toda a aplicação. A equipe precisa provar a decisão com um teste pequeno e reversível. Um portal entrega imagens e arquivos estáticos do S3 a usuários globais. A origem recebe leituras repetidas e usuários distantes observam alta latência. Qual mudança melhora desempenho com menor operação? Escolha a alternativa que atende diretamente ao requisito.
...
- **C)** Aumentar o tamanho do bucket S3.
- **D)** Copiar manualmente todos os objetos para volumes EBS em cada região.
...
- **C:** Incorreta — S3 não precisa de provisionamento de capacidade do bucket e isso não aproxima conteúdo do usuário.
- **D:** Incorreta — Isso cria operação, inconsistência e não fornece uma rede de borda global.
...
## 5. EFS lifecycle e Infrequent Access

> **ID:** `SIM01-Q005` · **Domínio:** D4 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A rede de hotéis brisa precisa corrigir uma descoberta da revisão Well-Architected. O tráfego normal é estável, mas cresce oito vezes em campanhas. Um sistema EFS Regional contém muitos arquivos antigos raramente lidos, mas eles ainda precisam aparecer no mesmo namespace POSIX. Como reduzir o custo sem migração manual? Considere o principal trade-off operacional e escolha a melhor solução.

- **A)** Criar um EFS novo para cada arquivo frio.
- **B)** Configurar EFS lifecycle management para mover arquivos não acessados a IA/Archive conforme elegibilidade e, se adequado, voltar ao Standard no primeiro acesso.
...
- **B:** Correta — EFS lifecycle move dados frios para classes mais baratas mantendo o mesmo sistema de arquivos e acesso transparente à aplicação.
...
- **A:** Incorreta — URLs pré-assinadas expiram e não substituem uma arquitetura de distribuição, cache e proteção de borda.
...
## 7. ALB e Auto Scaling multi-AZ

> **ID:** `SIM01-Q007` · **Domínio:** D2 · **Dificuldade:** difícil · **Tempo:** 130 s · **Pontos:** 3

A varejista nuvem sul precisa atender a uma auditoria sem redesenhar toda a aplicação. O tráfego normal é estável, mas cresce oito vezes em campanhas. Uma aplicação HTTP stateless precisa continuar disponível quando uma instância ou uma AZ falhar e deve ajustar capacidade conforme o volume de requisições. Qual arquitetura escolher? Priorize simultaneamente o requisito explícito, o menor acoplamento operacional e o comportamento em falhas.

- **A)** Executar uma única instância grande e reiniciá-la com um cron job.
...
- **A:** Incorreta — Ainda há ponto único de falha e recuperação dependente de automação frágil.
...
> **ID:** `SIM01-Q008` · **Domínio:** D3 · **Dificuldade:** fácil · **Tempo:** 90 s · **Pontos:** 1

A empresa de logística rota certa precisa corrigir uma descoberta da revisão Well-Architected. A aplicação é global e a latência percebida pelo usuário é uma métrica de negócio. Uma API lê repetidamente os mesmos registros de sessão e catálogo em um banco relacional. O banco está no limite de CPU, e dados em cache podem expirar em minutos. Qual solução reduz a latência? Escolha a alternativa que atende diretamente ao requisito.
...
> **ID:** `SIM01-Q009` · **Domínio:** D4 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

O banco digital pioneiro está separando produção e desenvolvimento em contas distintas. Uma interrupção de poucos minutos gera impacto financeiro mensurável. Logs são acessados diariamente por 30 dias, raramente até o primeiro ano e precisam ser retidos por sete anos. A equipe quer reduzir custo automaticamente sem scripts. Qual abordagem usar? Considere o principal trade-off operacional e escolha a melhor solução.

- **A)** Excluir os logs aos 30 dias apesar da retenção de sete anos.
...
- **A:** Incorreta — Isso viola o requisito regulatório.
...
## 10. Criptografia com AWS KMS

> **ID:** `SIM01-Q010` · **Domínio:** D1 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A seguradora atlântico precisa atender a uma auditoria sem redesenhar toda a aplicação. A equipe precisa provar a decisão com um teste pequeno e reversível. Um banco RDS deve ser criptografado em repouso com uma chave controlada pela empresa. A segurança exige separação de funções, auditoria de uso da chave e rotação anual automática. Qual desenho é apropriado? Considere o principal trade-off operacional e escolha a melhor solução.

- **A)** Guardar uma senha AES em user data e criptografar manualmente cada página do banco.
...
- **D)** Usar uma chave gerenciada pela AWS e editar diretamente sua key policy para impor a separação de funções.
...
- **A:** Incorreta — User data não é cofre de segredos e criptografia manual acrescenta risco e operação desnecessários.
...
- **D:** Incorreta — Key policies de chaves gerenciadas pela AWS não são editáveis pelo cliente como as de uma chave gerenciada pelo cliente.
...
> **ID:** `SIM01-Q011` · **Domínio:** D2 · **Dificuldade:** fácil · **Tempo:** 90 s · **Pontos:** 1

A proptech janela precisa corrigir uma descoberta da revisão Well-Architected. Uma interrupção de poucos minutos gera impacto financeiro mensurável. Uma aplicação possui um endpoint primário em uma região e um site de recuperação em outra. O DNS deve enviar tráfego ao secundário apenas quando o primário não estiver saudável. Qual política usar? Escolha a alternativa que atende diretamente ao requisito.

- **A)** Aumentar o TTL para 24 horas para acelerar a mudança.
...
- **A:** Incorreta — TTL alto faz caches manterem a resposta anterior por mais tempo, retardando a convergência.
...
## 12. Chave de partição do DynamoDB

> **ID:** `SIM01-Q012` · **Domínio:** D3 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A fabricante serra azul está separando produção e desenvolvimento em contas distintas. O tráfego normal é estável, mas cresce oito vezes em campanhas. Uma tabela DynamoDB recebe gravações intensas. A chave de partição atual usa apenas o código de um país, e um país concentra 80% do tráfego, causando throttling. Qual redesign é mais apropriado? Considere o principal trade-off operacional e escolha a melhor solução.
...
## 13. Savings Plans e Spot

> **ID:** `SIM01-Q013` · **Domínio:** D4 · **Dificuldade:** difícil · **Tempo:** 130 s · **Pontos:** 3

A startup ipê digital precisa atender a uma auditoria sem redesenhar toda a aplicação. Os dados incluem informações reguladas e devem permanecer auditáveis. Uma plataforma tem uma base de compute estável 24x7 e jobs batch tolerantes a interrupção durante a madrugada. Qual combinação tende a otimizar custo sem comprometer a base? Priorize simultaneamente o requisito explícito, o menor acoplamento operacional e o comportamento em falhas.
...
## 14. Rotação de credenciais com Secrets Manager

> **ID:** `SIM01-Q014` · **Domínio:** D1 · **Dificuldade:** fácil · **Tempo:** 90 s · **Pontos:** 1

A edtech farol precisa corrigir uma descoberta da revisão Well-Architected. A aplicação é global e a latência percebida pelo usuário é uma métrica de negócio. Uma aplicação usa credenciais de um banco RDS. A senha precisa rotacionar automaticamente a cada 30 dias sem ser gravada no código ou na imagem do contêiner. Qual é a solução com menor esforço operacional? Escolha a alternativa que atende diretamente ao requisito.

- **A)** Salvar a senha em uma variável de ambiente no Dockerfile e recriar a imagem mensalmente.
- **B)** Guardar a senha em uma tag do recurso RDS protegida por IAM.
- **C)** Armazenar a credencial no AWS Secrets Manager, configurar rotação com Lambda e conceder à task role permissão de leitura do segredo.
- **D)** Criar access keys para o usuário raiz e usá-las como senha do banco.
...
- **A:** Incorreta — A imagem pode vazar a senha e a rotação permanece manual e acoplada ao deploy.
- **B:** Incorreta — Tags não são um mecanismo de armazenamento seguro de segredos.
- **C:** Correta — Secrets Manager armazena, versiona e integra a rotação de segredos; a role do workload elimina credenciais AWS embutidas no contêiner.
- **D:** Incorreta — Credenciais AWS não substituem credenciais do banco e o usuário raiz não deve ser usado por workloads.
...
## 15. Desacoplamento com SQS e DLQ

> **ID:** `SIM01-Q015` · **Domínio:** D2 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A indústria cedro está separando produção e desenvolvimento em contas distintas. Os dados incluem informações reguladas e devem permanecer auditáveis. Pedidos chegam em rajadas e o processador pode ficar temporariamente indisponível. Nenhum pedido pode ser perdido, falhas repetidas devem ser isoladas e o produtor não deve esperar o processamento. Qual desenho usar? Considere o principal trade-off operacional e escolha a melhor solução.

- **A)** Publicar somente em uma SNS topic sem qualquer assinatura durável.
- **B)** Gravar pedidos em instance store de uma única EC2.
- **C)** Fazer o produtor chamar o processador de forma síncrona com retries infinitos.
...
- **A:** Incorreta — SNS sozinho não mantém backlog para um consumidor indisponível; uma assinatura SQS adicionaria durabilidade.
- **B:** Incorreta — Instance store é efêmero e a instância continua sendo ponto único de falha.
- **C:** Incorreta — Isso acopla os componentes, prende recursos e pode criar tempestades de retries.
...
## 16. Read replicas para escalar leituras

> **ID:** `SIM01-Q016` · **Domínio:** D3 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A rede hospitalar horizonte precisa atender a uma auditoria sem redesenhar toda a aplicação. Uma interrupção de poucos minutos gera impacto financeiro mensurável. Um banco RDS atende relatórios pesados que disputam CPU e I/O com transações. Os relatórios toleram alguns segundos de defasagem e a alta disponibilidade já está coberta. O que fazer? Considere o principal trade-off operacional e escolha a melhor solução.

- **A)** Criar read replicas e direcionar consultas de relatório aos endpoints de leitura, monitorando replication lag.
- **B)** Aumentar o TTL do Route 53 do endpoint do banco.
- **C)** Usar o standby Multi-AZ diretamente para todas as consultas de relatório.
...
- **A:** Correta — Read replicas usam replicação assíncrona para retirar consultas de leitura do primário; a tolerância a defasagem torna a solução adequada.
- **B:** Incorreta — TTL DNS não reduz a carga de consultas no primário.
- **C:** Incorreta — Na implantação Multi-AZ tradicional, o standby não é endpoint de leitura da aplicação.
...
## 17. RDS Reserved DB Instances

> **ID:** `SIM01-Q017` · **Domínio:** D4 · **Dificuldade:** difícil · **Tempo:** 130 s · **Pontos:** 3

O marketplace beija-flor precisa corrigir uma descoberta da revisão Well-Architected. A solução atual funciona, mas não atende ao novo requisito não funcional. Um banco RDS de produção possui classe e região estáveis, funciona continuamente e deve permanecer assim por pelo menos um ano. A empresa quer desconto sem redesenhar a aplicação. O que avaliar? Priorize simultaneamente o requisito explícito, o menor acoplamento operacional e o comportamento em falhas.
...
## 18. VPC Gateway Endpoint para S3

> **ID:** `SIM01-Q018` · **Domínio:** D1 · **Dificuldade:** fácil · **Tempo:** 90 s · **Pontos:** 1

A fintech aurora está separando produção e desenvolvimento em contas distintas. O tráfego normal é estável, mas cresce oito vezes em campanhas. Instâncias EC2 em subnets privadas enviam grandes volumes ao S3. O tráfego não pode atravessar a internet e a solução deve evitar cobrança por hora de um componente de rede. Qual opção usar? Escolha a alternativa que atende diretamente ao requisito.

- **A)** Atribuir IPv4 público às instâncias e permitir saída 0.0.0.0/0.
...
- **D)** Criar um Interface Endpoint para qualquer serviço e esperar que ele seja sempre gratuito.
...
- **A:** Incorreta — Isso expõe as instâncias a uma rota de internet e viola o requisito de tráfego privado.
...
- **D:** Incorreta — Interface endpoints têm cobrança por hora e processamento de dados; a afirmação de gratuidade é incorreta.
...
## 19. Fan-out com SNS e SQS

> **ID:** `SIM01-Q019` · **Domínio:** D2 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A universidade saber precisa atender a uma auditoria sem redesenhar toda a aplicação. A solução atual funciona, mas não atende ao novo requisito não funcional. Cada evento de pedido deve ser processado independentemente por faturamento, estoque e analytics. Se um consumidor parar, os outros devem continuar e o backlog daquele consumidor deve ser preservado. Qual arquitetura é apropriada? Considere o principal trade-off operacional e escolha a melhor solução.

- **A)** Usar uma única fila SQS e fazer os três serviços competirem pela mesma mensagem.
- **B)** Chamar os três serviços sequencialmente dentro do produtor.
- **C)** Guardar eventos em logs locais da instância produtora.
...
- **A:** Incorreta — Consumidores concorrentes em uma fila recebem mensagens diferentes; não há fan-out por consumidor.
- **B:** Incorreta — A falha ou lentidão de um serviço afeta todos e acopla o produtor aos consumidores.
- **C:** Incorreta — Logs locais não são um canal durável e consumível de integração.
...
## 20. Tipos de volume EBS

> **ID:** `SIM01-Q020` · **Domínio:** D3 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A foodtech panela precisa corrigir uma descoberta da revisão Well-Architected. Os dados incluem informações reguladas e devem permanecer auditáveis. Um banco autogerenciado em EC2 exige latência de I/O consistente e dezenas de milhares de IOPS sustentadas. A performance deve ser previsível, mesmo que custe mais que uso geral. Qual volume escolher? Considere o principal trade-off operacional e escolha a melhor solução.

- **A)** Usar EBS io2, dimensionando IOPS e throughput conforme a instância e a carga, e validar limites ponta a ponta.
- **B)** Usar st1 para milhões de pequenos I/Os aleatórios.
- **C)** Usar instance store sem replicação porque todo dado EBS é efêmero.
- **D)** Usar sc1 porque é o volume com menor latência para bancos críticos.
...
- **A:** Correta — io2 é SSD de Provisioned IOPS para workloads críticos que precisam de desempenho consistente e alta durabilidade.
- **B:** Incorreta — st1 é otimizado a throughput sequencial, não IOPS aleatórios de banco.
- **C:** Incorreta — EBS é persistente; instance store é que é efêmero e exigiria proteção adicional.
- **D:** Incorreta — sc1 é HDD de baixo custo para acesso pouco frequente e não oferece latência SSD.
...
## 21. Custo de NAT e VPC endpoints

> **ID:** `SIM01-Q021` · **Domínio:** D4 · **Dificuldade:** fácil · **Tempo:** 90 s · **Pontos:** 1

A varejista nuvem sul está separando produção e desenvolvimento em contas distintas. O orçamento é limitado e a operação manual deve ser mínima. Instâncias privadas baixam terabytes mensalmente do S3 através de NAT Gateways. A fatura mostra alto processamento de dados no NAT. Como reduzir o custo mantendo caminho privado? Escolha a alternativa que atende diretamente ao requisito.
...
- **D)** Adicionar mais NAT Gateways e continuar roteando S3 por todos eles.
...
- **D:** Incorreta — Isso pode melhorar resiliência, mas não remove a cobrança de processamento responsável pelo custo.
...
## 22. SCP em AWS Organizations

> **ID:** `SIM01-Q022` · **Domínio:** D1 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A empresa de geoprocessamento mapa vivo precisa atender a uma auditoria sem redesenhar toda a aplicação. Uma interrupção de poucos minutos gera impacto financeiro mensurável. A organização quer impedir que qualquer conta membro desative o CloudTrail ou crie recursos fora de regiões aprovadas, inclusive quando um administrador local concede Allow. Qual controle deve formar o guardrail? Considere o principal trade-off operacional e escolha a melhor solução.

- **A)** Usar somente security groups para bloquear chamadas de API fora da região.
...
- **D)** Adicionar uma IAM policy Allow a todos os administradores das contas.
...
- **A:** Incorreta — Security groups filtram tráfego de recursos e não governam permissões de APIs ou regiões.
...
- **D:** Incorreta — Um Allow amplia permissões locais e não cria o guardrail organizacional solicitado.
...
## 23. Estratégias de disaster recovery

> **ID:** `SIM01-Q023` · **Domínio:** D2 · **Dificuldade:** difícil · **Tempo:** 130 s · **Pontos:** 3

A rede de hotéis brisa precisa corrigir uma descoberta da revisão Well-Architected. O orçamento é limitado e a operação manual deve ser mínima. Um sistema regional exige RTO de 15 minutos e RPO de poucos minutos. A empresa aceita manter capacidade reduzida ativa na região de recuperação, mas não quer pagar por uma cópia em escala total. Qual estratégia se encaixa melhor? Priorize simultaneamente o requisito explícito, o menor acoplamento operacional e o comportamento em falhas.

- **A)** Multi-site active-active em escala total obrigatoriamente.
- **B)** Backup and restore com backups semanais offline.
...
- **B:** Incorreta — Em geral não atende RPO de minutos nem RTO de 15 minutos.
...
> **ID:** `SIM01-Q025` · **Domínio:** D4 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A proptech janela precisa atender a uma auditoria sem redesenhar toda a aplicação. A carga atende milhares de requisições por segundo em horários de pico. Centenas de instâncias EC2 têm baixa utilização, mas a equipe não sabe quais podem ser reduzidas sem risco de memória ou performance. Ela quer recomendações baseadas em métricas antes de mudar tamanhos. Qual serviço usar? Considere o principal trade-off operacional e escolha a melhor solução.

- **A)** Reduzir todas as instâncias para t3.micro sem medir.
...
- **A:** Incorreta — Uma regra única ignora CPU, memória, rede, burst e requisitos diferentes.
...
## 26. Trilha de auditoria organizacional

> **ID:** `SIM01-Q026` · **Domínio:** D1 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A empresa de energia raio precisa corrigir uma descoberta da revisão Well-Architected. Os dados incluem informações reguladas e devem permanecer auditáveis. Auditores exigem registro centralizado e resistente a adulteração das chamadas de API de todas as contas atuais e futuras. Também é preciso alertar quando alguém tentar apagar uma trilha. Qual arquitetura é adequada? Considere o principal trade-off operacional e escolha a melhor solução.
...
- **D)** Habilitar logs de acesso do ALB para registrar alterações no IAM.
...
- **D:** Incorreta — Logs do ALB mostram requisições ao balanceador, não alterações administrativas em serviços AWS.
...
## 27. Replicação S3 entre regiões

> **ID:** `SIM01-Q027` · **Domínio:** D2 · **Dificuldade:** difícil · **Tempo:** 130 s · **Pontos:** 3

O banco digital pioneiro está separando produção e desenvolvimento em contas distintas. A carga atende milhares de requisições por segundo em horários de pico. Objetos de um bucket precisam ser copiados automaticamente para outra região para recuperação. A empresa também quer preservar versões e impedir que uma exclusão acidental destrua imediatamente a cópia histórica. O que configurar? Priorize simultaneamente o requisito explícito, o menor acoplamento operacional e o comportamento em falhas.

- **A)** Montar o bucket como EBS e fazer snapshot da instância.
- **B)** Desativar versionamento para reduzir o número de cópias.
...
- **A:** Incorreta — S3 não é montado como volume EBS, e snapshot de EC2 não protege objetos do bucket.
- **B:** Incorreta — CRR exige versionamento e desativá-lo prejudica recuperação de alterações/exclusões.
...
## 28. AWS Global Accelerator

> **ID:** `SIM01-Q028` · **Domínio:** D3 · **Dificuldade:** fácil · **Tempo:** 90 s · **Pontos:** 1

A healthtech pulso precisa atender a uma auditoria sem redesenhar toda a aplicação. O orçamento é limitado e a operação manual deve ser mínima. Um aplicativo de jogos usa TCP/UDP, possui endpoints em duas regiões e precisa de IPs anycast estáticos, failover rápido e tráfego pela rede global da AWS. O conteúdo não é cacheável. Qual serviço usar? Escolha a alternativa que atende diretamente ao requisito.
...
## 29. Capacidade do DynamoDB

> **ID:** `SIM01-Q029` · **Domínio:** D4 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A indústria cedro precisa corrigir uma descoberta da revisão Well-Architected. A equipe precisa provar a decisão com um teste pequeno e reversível. Uma tabela nova tem tráfego imprevisível e pode ficar horas ociosa antes de picos abruptos. A equipe não conhece a capacidade necessária e quer evitar administração inicial. Qual modo escolher? Considere o principal trade-off operacional e escolha a melhor solução.
...
## 30. Security groups e network ACLs

> **ID:** `SIM01-Q030` · **Domínio:** D1 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2
...
- **A)** Associar uma NACL diretamente a cada instância EC2.
...
- **D)** Usar security group para negar explicitamente um CIDR malicioso.
...
- **A:** Incorreta — NACLs são associadas a subnets; security groups são associados às interfaces de rede.
...
- **D:** Incorreta — Security groups têm regras de allow, não regras de deny explícito.
...
## 31. Amazon EFS Regional

> **ID:** `SIM01-Q031` · **Domínio:** D2 · **Dificuldade:** fácil · **Tempo:** 90 s · **Pontos:** 1

A startup ipê digital precisa atender a uma auditoria sem redesenhar toda a aplicação. A equipe precisa provar a decisão com um teste pequeno e reversível. Um conjunto de instâncias Linux em várias AZs precisa compartilhar arquivos POSIX. Os dados devem permanecer acessíveis quando uma instância ou uma AZ falhar, sem gerenciar servidores de arquivos. Qual armazenamento usar? Escolha a alternativa que atende diretamente ao requisito.
...
- **D)** Amazon EFS Regional montado pelos clientes nas diferentes AZs, com mount targets e security groups adequados.
...
- **D:** Correta — EFS Regional fornece sistema de arquivos gerenciado, elástico e multi-AZ, adequado ao compartilhamento simultâneo entre instâncias Linux.
...
> **ID:** `SIM01-Q032` · **Domínio:** D3 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A empresa de jogos capivara precisa corrigir uma descoberta da revisão Well-Architected. A carga atende milhares de requisições por segundo em horários de pico. Um workload HPC em milhares de vCPUs processa um dataset grande do S3 e precisa de sistema de arquivos paralelo com throughput muito alto. Ao final, os resultados devem voltar ao S3. Qual serviço usar? Considere o principal trade-off operacional e escolha a melhor solução.
...
- **C)** Executar um servidor SMB t3.micro como ponto central.
...
- **C:** Incorreta — O servidor seria gargalo e ponto único, além de protocolo inadequado ao padrão HPC descrito.
...
## 33. Snapshots EBS com Data Lifecycle Manager

> **ID:** `SIM01-Q033` · **Domínio:** D4 · **Dificuldade:** difícil · **Tempo:** 130 s · **Pontos:** 3

A universidade saber está separando produção e desenvolvimento em contas distintas. A aplicação é global e a latência percebida pelo usuário é uma métrica de negócio. Volumes EBS precisam de snapshots diários, retenção por 35 dias e exclusão automática dos antigos. O processo atual usa scripts em uma instância que frequentemente falha. Qual opção reduz operação e custo? Priorize simultaneamente o requisito explícito, o menor acoplamento operacional e o comportamento em falhas.
...
## 34. Autenticação de clientes com Amazon Cognito

> **ID:** `SIM01-Q034` · **Domínio:** D1 · **Dificuldade:** fácil · **Tempo:** 90 s · **Pontos:** 1

A agência pública portal aberto precisa atender a uma auditoria sem redesenhar toda a aplicação. O orçamento é limitado e a operação manual deve ser mínima. Um aplicativo móvel voltado a milhões de consumidores precisa cadastro, login, recuperação de senha, MFA opcional e tokens para chamar uma API. A equipe não quer manter um diretório próprio. Qual serviço é indicado? Escolha a alternativa que atende diretamente ao requisito.

- **A)** Salvar senhas em uma tabela DynamoDB sem hash e validar na Lambda.
...
- **A:** Incorreta — Armazenamento de senhas em texto é inseguro e recria capacidades já gerenciadas pelo Cognito.
...
## 35. Aurora Global Database

> **ID:** `SIM01-Q035` · **Domínio:** D2 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

O marketplace beija-flor precisa corrigir uma descoberta da revisão Well-Architected. A aplicação é global e a latência percebida pelo usuário é uma métrica de negócio. Uma aplicação de leitura global baseada em Aurora precisa latência local em regiões secundárias e recuperação regional com replicação rápida. Escritas permanecem centralizadas durante operação normal. Qual recurso escolher? Considere o principal trade-off operacional e escolha a melhor solução.

- **A)** Colocar o endpoint regional atrás de CloudFront para armazenar respostas SQL em cache.
- **B)** Usar apenas Multi-AZ no cluster primário e esperar endpoints de leitura em outras regiões.
- **C)** Exportar snapshots manualmente uma vez por mês.
- **D)** Usar Aurora Global Database com cluster primário gravável e clusters secundários de leitura nas regiões necessárias.
...
- **A:** Incorreta — CloudFront não se conecta diretamente a um protocolo de banco e não cria réplicas do Aurora.
- **B:** Incorreta — Multi-AZ oferece resiliência regional, não clusters de leitura entre regiões.
- **C:** Incorreta — Snapshots mensais não oferecem replicação contínua nem RPO/RTO compatíveis.
- **D:** Correta — Aurora Global Database usa replicação dedicada entre regiões, oferece leituras locais e permite promover uma região secundária em um evento de recuperação.
...
## 36. Amazon Kinesis Data Streams

> **ID:** `SIM01-Q036` · **Domínio:** D3 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A plataforma de mídia onda está separando produção e desenvolvimento em contas distintas. A equipe precisa provar a decisão com um teste pequeno e reversível. Sensores enviam eventos continuamente e vários consumidores precisam processar o mesmo stream com baixa latência, preservando ordem por dispositivo e possibilidade de replay dentro da retenção. Qual serviço escolher? Considere o principal trade-off operacional e escolha a melhor solução.
...
- **C)** Gravar eventos em arquivos locais e copiá-los semanalmente.
- **D)** Usar SNS sem assinantes duráveis nem retenção.
...
- **C:** Incorreta — Isso não atende baixa latência, durabilidade gerenciada ou múltiplos consumidores.
- **D:** Incorreta — SNS faz push, mas sozinho não fornece retenção e replay do stream.
...
## 37. AWS Budgets e Cost Explorer

> **ID:** `SIM01-Q037` · **Domínio:** D4 · **Dificuldade:** difícil · **Tempo:** 130 s · **Pontos:** 3

A rede de hotéis brisa precisa atender a uma auditoria sem redesenhar toda a aplicação. O tráfego normal é estável, mas cresce oito vezes em campanhas. FinOps precisa alertar quando a previsão mensal ultrapassar o orçamento e depois investigar quais serviços e tags explicam a variação. Qual combinação usar? Priorize simultaneamente o requisito explícito, o menor acoplamento operacional e o comportamento em falhas.
...
- **D)** Esperar a fatura fechar e analisar apenas uma vez por ano.
...
- **D:** Incorreta — Isso elimina alerta precoce e capacidade de correção durante o período.
...
## 38. Proteção de camada 7 com AWS WAF

> **ID:** `SIM01-Q038` · **Domínio:** D1 · **Dificuldade:** fácil · **Tempo:** 90 s · **Pontos:** 1

A cooperativa campo vivo precisa corrigir uma descoberta da revisão Well-Architected. A carga atende milhares de requisições por segundo em horários de pico. Uma API pública atrás de um ALB sofre credential stuffing e rajadas de um pequeno conjunto de endereços IP. É necessário bloquear padrões HTTP e limitar requisições por origem sem alterar a aplicação. O que fazer? Escolha a alternativa que atende diretamente ao requisito.
...
- **D)** Aumentar o número de instâncias do Auto Scaling sem filtrar as requisições.
...
- **D:** Incorreta — Escalar pode absorver carga, mas não bloqueia abuso e ainda amplia custo.
...
## 40. Políticas de Auto Scaling

> **ID:** `SIM01-Q040` · **Domínio:** D3 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2
...
- **A:** Incorreta — EBS provisionado seria mais caro e não oferece a semântica/escala de armazenamento de objetos.
- **B:** Correta — Intelligent-Tiering monitora acesso e move objetos entre tiers automáticos sem cobrança de recuperação nos tiers de acesso frequente/infrequente, cobrando pequena taxa de monitoramento.
...
## 42. S3 Block Public Access e políticas

> **ID:** `SIM01-Q042` · **Domínio:** D1 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A seguradora atlântico está separando produção e desenvolvimento em contas distintas. A equipe precisa provar a decisão com um teste pequeno e reversível. Uma empresa armazena relatórios confidenciais em centenas de buckets. Ela precisa impedir exposição pública acidental em toda a organização e permitir acesso apenas por um VPC endpoint específico. Qual desenho é mais seguro? Considere o principal trade-off operacional e escolha a melhor solução.

- **A)** Criptografar os objetos, mas deixar leitura pública para qualquer principal.
...
- **D)** Associar um security group diretamente ao bucket.
...
- **A:** Incorreta — Criptografia em repouso não corrige uma autorização pública; serviços autorizados ainda descriptografariam dados.
...
- **D:** Incorreta — Buckets S3 não aceitam security groups.
...
## 43. Falhas assíncronas do Lambda

> **ID:** `SIM01-Q043` · **Domínio:** D2 · **Dificuldade:** difícil · **Tempo:** 130 s · **Pontos:** 3

A proptech janela precisa atender a uma auditoria sem redesenhar toda a aplicação. Uma interrupção de poucos minutos gera impacto financeiro mensurável. Uma função Lambda é invocada de modo assíncrono por eventos. Após as tentativas automáticas, eventos malsucedidos devem ser preservados para investigação e reprocessamento, sem bloquear eventos saudáveis. O que configurar? Priorize simultaneamente o requisito explícito, o menor acoplamento operacional e o comportamento em falhas.

- **A)** Definir timeout infinito para garantir que toda execução termine.
...
- **A:** Incorreta — Lambda tem limite de timeout e aumentar duração não resolve eventos permanentemente inválidos.
...
## 44. Lambda provisioned concurrency

> **ID:** `SIM01-Q044` · **Domínio:** D3 · **Dificuldade:** fácil · **Tempo:** 90 s · **Pontos:** 1

A fabricante serra azul precisa corrigir uma descoberta da revisão Well-Architected. O tráfego normal é estável, mas cresce oito vezes em campanhas. Uma função Lambda síncrona atende uma API sensível à latência. Após períodos ociosos, cold starts ultrapassam o SLA; o volume do horário comercial é previsível. Qual recurso reduz essa variação? Escolha a alternativa que atende diretamente ao requisito.
...
## 46. Permissions boundaries e delegação

> **ID:** `SIM01-Q046` · **Domínio:** D1 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A edtech farol precisa atender a uma auditoria sem redesenhar toda a aplicação. A aplicação é global e a latência percebida pelo usuário é uma métrica de negócio. Uma plataforma permite que times de produto criem suas próprias roles, mas segurança precisa garantir que nenhuma role criada ultrapasse um conjunto máximo de ações. Os times ainda devem escolher permissões dentro desse limite. Qual recurso usar? Considere o principal trade-off operacional e escolha a melhor solução.

- **A)** Usar uma NACL para limitar quais APIs IAM podem ser chamadas.
...
- **D)** Anexar AdministratorAccess e pedir que cada time não use ações perigosas.
...
- **A:** Incorreta — NACLs filtram pacotes e não avaliam ações IAM.
...
- **D:** Incorreta — Confiança processual não impõe um limite técnico e AdministratorAccess viola privilégio mínimo.
...
## 47. Políticas centralizadas com AWS Backup

> **ID:** `SIM01-Q047` · **Domínio:** D2 · **Dificuldade:** difícil · **Tempo:** 130 s · **Pontos:** 3

A indústria cedro precisa corrigir uma descoberta da revisão Well-Architected. Os dados incluem informações reguladas e devem permanecer auditáveis. Dezenas de contas precisam de backups consistentes de EBS, RDS e EFS, cópias entre regiões e proteção contra exclusão pela conta de origem. Segurança quer governança central. Qual abordagem usar? Priorize simultaneamente o requisito explícito, o menor acoplamento operacional e o comportamento em falhas.

- **A)** Copiar arquivos para o disco local de uma instância na mesma conta.
...
- **A:** Incorreta — A cópia permanece vulnerável a falhas e exclusões na mesma fronteira administrativa.
...
## 48. Athena, partições e formato colunar

> **ID:** `SIM01-Q048` · **Domínio:** D3 · **Dificuldade:** fácil · **Tempo:** 90 s · **Pontos:** 1

A rede hospitalar horizonte está separando produção e desenvolvimento em contas distintas. Uma interrupção de poucos minutos gera impacto financeiro mensurável. Consultas Athena leem logs no S3 particionados apenas em arquivos JSON grandes e escaneiam terabytes para filtrar um único dia e região. Como reduzir tempo e bytes processados? Escolha a alternativa que atende diretamente ao requisito.
...
- **C)** Usar SELECT * em todas as consultas para aumentar o cache.
...
- **C:** Incorreta — Ler todas as colunas aumenta dados examinados e normalmente piora custo e latência.
...
> **ID:** `SIM01-Q049` · **Domínio:** D1 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A plataforma de ingressos palco precisa atender a uma auditoria sem redesenhar toda a aplicação. Uma interrupção de poucos minutos gera impacto financeiro mensurável. Uma aplicação na conta de produção precisa ler objetos de um bucket pertencente à conta de dados, sem armazenar chaves de acesso. O acesso deve ser temporário, auditável e seguir privilégio mínimo. Qual solução atende melhor? Considere o principal trade-off operacional e escolha a melhor solução.

- **A)** Compartilhar a senha do usuário raiz da conta de dados pelo Secrets Manager.
...
- **C)** Criar um usuário IAM na conta de dados e copiar access key e secret key para a instância.
- **D)** Tornar o bucket público e restringir o endereço IP na aplicação.
...
- **A:** Incorreta — Credenciais do usuário raiz não devem ser usadas por aplicações, mesmo quando armazenadas em um cofre.
...
- **C:** Incorreta — Chaves de longa duração aumentam o risco de vazamento e exigem rotação; não são necessárias para workloads na AWS.
- **D:** Incorreta — Exposição pública viola o requisito e endereço IP não substitui autenticação e autorização.
...
> **ID:** `SIM01-Q050` · **Domínio:** D2 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A agência pública portal aberto precisa corrigir uma descoberta da revisão Well-Architected. O orçamento é limitado e a operação manual deve ser mínima. Um banco transacional RDS precisa recuperar automaticamente de falha de instância ou de AZ, preservando o mesmo endpoint. A carga de leitura não é o problema principal. Qual configuração atende ao requisito? Considere o principal trade-off operacional e escolha a melhor solução.
...
## 51. Cache global com CloudFront

> **ID:** `SIM01-Q051` · **Domínio:** D3 · **Dificuldade:** fácil · **Tempo:** 90 s · **Pontos:** 1

A universidade saber está separando produção e desenvolvimento em contas distintas. A solução atual funciona, mas não atende ao novo requisito não funcional. Um portal entrega imagens e arquivos estáticos do S3 a usuários globais. A origem recebe leituras repetidas e usuários distantes observam alta latência. Qual mudança melhora desempenho com menor operação? Escolha a alternativa que atende diretamente ao requisito.
...
## 52. S3 privado atrás do CloudFront

> **ID:** `SIM01-Q052` · **Domínio:** D1 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

O banco digital pioneiro precisa atender a uma auditoria sem redesenhar toda a aplicação. A solução atual funciona, mas não atende ao novo requisito não funcional. Um site estático precisa ser distribuído globalmente. Os objetos do S3 não podem ser acessados diretamente pela internet, e a equipe quer aplicar regras contra requisições maliciosas na borda. O que deve ser configurado? Considere o principal trade-off operacional e escolha a melhor solução. Para rastreabilidade, o comitê registrou este caso como SIM01-Q052.
...
## 53. ALB e Auto Scaling multi-AZ

> **ID:** `SIM01-Q053` · **Domínio:** D2 · **Dificuldade:** difícil · **Tempo:** 130 s · **Pontos:** 3

A fabricante serra azul precisa corrigir uma descoberta da revisão Well-Architected. A equipe precisa provar a decisão com um teste pequeno e reversível. Uma aplicação HTTP stateless precisa continuar disponível quando uma instância ou uma AZ falhar e deve ajustar capacidade conforme o volume de requisições. Qual arquitetura escolher? Priorize simultaneamente o requisito explícito, o menor acoplamento operacional e o comportamento em falhas.
...
## 54. Criptografia com AWS KMS

> **ID:** `SIM01-Q054` · **Domínio:** D1 · **Dificuldade:** fácil · **Tempo:** 90 s · **Pontos:** 1

A foodtech panela está separando produção e desenvolvimento em contas distintas. A equipe precisa provar a decisão com um teste pequeno e reversível. Um banco RDS deve ser criptografado em repouso com uma chave controlada pela empresa. A segurança exige separação de funções, auditoria de uso da chave e rotação anual automática. Qual desenho é apropriado? Escolha a alternativa que atende diretamente ao requisito.

- **A)** Usar uma chave gerenciada pela AWS e editar diretamente sua key policy para impor a separação de funções.
- **B)** Guardar uma senha AES em user data e criptografar manualmente cada página do banco.
- **C)** Criar uma chave simétrica gerenciada pelo cliente no AWS KMS, limitar key policy/grants, habilitar rotação e selecioná-la ao criar o RDS.
- **D)** Habilitar apenas TLS no endpoint do RDS.
...
- **A:** Incorreta — Key policies de chaves gerenciadas pela AWS não são editáveis pelo cliente como as de uma chave gerenciada pelo cliente.
- **B:** Incorreta — User data não é cofre de segredos e criptografia manual acrescenta risco e operação desnecessários.
- **C:** Correta — Uma chave KMS gerenciada pelo cliente oferece controle de política, trilha de auditoria no CloudTrail e rotação automática compatível com a criptografia do RDS.
- **D:** Incorreta — TLS protege dados em trânsito; não satisfaz o requisito de criptografia em repouso.
...
## 55. Route 53 failover

> **ID:** `SIM01-Q055` · **Domínio:** D2 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A companhia aérea ventos precisa atender a uma auditoria sem redesenhar toda a aplicação. Uma interrupção de poucos minutos gera impacto financeiro mensurável. Uma aplicação possui um endpoint primário em uma região e um site de recuperação em outra. O DNS deve enviar tráfego ao secundário apenas quando o primário não estiver saudável. Qual política usar? Considere o principal trade-off operacional e escolha a melhor solução.

- **A)** Usar geolocation apenas, pois localização detecta falha automaticamente.
- **B)** Aumentar o TTL para 24 horas para acelerar a mudança.
- **C)** Usar simple routing com dois endereços e nenhum health check.
- **D)** Criar registros Route 53 com failover routing, marcar primário/secundário e associar health check ao endpoint primário.
...
- **B:** Incorreta — TTL alto faz caches manterem a resposta anterior por mais tempo, retardando a convergência.
- **C:** Incorreta — Simple routing não fornece o comportamento primário/secundário orientado por saúde.
- **D:** Correta — A política de failover usa a saúde do recurso para responder com o secundário quando o primário falha, implementando active-passive no DNS.
...
## 56. Rotação de credenciais com Secrets Manager

> **ID:** `SIM01-Q056` · **Domínio:** D1 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A cooperativa campo vivo precisa corrigir uma descoberta da revisão Well-Architected. Uma interrupção de poucos minutos gera impacto financeiro mensurável. Uma aplicação usa credenciais de um banco RDS. A senha precisa rotacionar automaticamente a cada 30 dias sem ser gravada no código ou na imagem do contêiner. Qual é a solução com menor esforço operacional? Considere o principal trade-off operacional e escolha a melhor solução.
...
- **B)** Salvar a senha em uma variável de ambiente no Dockerfile e recriar a imagem mensalmente.
- **C)** Guardar a senha em uma tag do recurso RDS protegida por IAM.
- **D)** Criar access keys para o usuário raiz e usá-las como senha do banco.
...
- **B:** Incorreta — A imagem pode vazar a senha e a rotação permanece manual e acoplada ao deploy.
- **C:** Incorreta — Tags não são um mecanismo de armazenamento seguro de segredos.
- **D:** Incorreta — Credenciais AWS não substituem credenciais do banco e o usuário raiz não deve ser usado por workloads.
...
## 57. VPC Gateway Endpoint para S3

> **ID:** `SIM01-Q057` · **Domínio:** D1 · **Dificuldade:** difícil · **Tempo:** 130 s · **Pontos:** 3

A companhia aérea ventos está separando produção e desenvolvimento em contas distintas. A carga atende milhares de requisições por segundo em horários de pico. Instâncias EC2 em subnets privadas enviam grandes volumes ao S3. O tráfego não pode atravessar a internet e a solução deve evitar cobrança por hora de um componente de rede. Qual opção usar? Priorize simultaneamente o requisito explícito, o menor acoplamento operacional e o comportamento em falhas.
...
- **C)** Enviar o tráfego por um NAT Gateway em cada AZ.
- **D)** Criar um Interface Endpoint para qualquer serviço e esperar que ele seja sempre gratuito.
...
- **C:** Incorreta — O NAT adiciona custo e o caminho usa o endpoint público; é desnecessário quando há gateway endpoint para S3.
- **D:** Incorreta — Interface endpoints têm cobrança por hora e processamento de dados; a afirmação de gratuidade é incorreta.
...
## 58. SCP em AWS Organizations

> **ID:** `SIM01-Q058` · **Domínio:** D1 · **Dificuldade:** fácil · **Tempo:** 90 s · **Pontos:** 1

A empresa de geoprocessamento mapa vivo precisa atender a uma auditoria sem redesenhar toda a aplicação. Uma interrupção de poucos minutos gera impacto financeiro mensurável. A organização quer impedir que qualquer conta membro desative o CloudTrail ou crie recursos fora de regiões aprovadas, inclusive quando um administrador local concede Allow. Qual controle deve formar o guardrail? Escolha a alternativa que atende diretamente ao requisito.
...
## 59. Trilha de auditoria organizacional

> **ID:** `SIM01-Q059` · **Domínio:** D1 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A edtech farol precisa corrigir uma descoberta da revisão Well-Architected. A carga atende milhares de requisições por segundo em horários de pico. Auditores exigem registro centralizado e resistente a adulteração das chamadas de API de todas as contas atuais e futuras. Também é preciso alertar quando alguém tentar apagar uma trilha. Qual arquitetura é adequada? Considere o principal trade-off operacional e escolha a melhor solução.
...
## 60. Desacoplamento com SQS e DLQ

> **ID:** `SIM01-Q060` · **Domínio:** D2 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2
...
## 61. ElastiCache for Redis

> **ID:** `SIM01-Q061` · **Domínio:** D3 · **Dificuldade:** fácil · **Tempo:** 90 s · **Pontos:** 1

A indústria cedro precisa atender a uma auditoria sem redesenhar toda a aplicação. A equipe precisa provar a decisão com um teste pequeno e reversível. Uma API lê repetidamente os mesmos registros de sessão e catálogo em um banco relacional. O banco está no limite de CPU, e dados em cache podem expirar em minutos. Qual solução reduz a latência? Escolha a alternativa que atende diretamente ao requisito.

- **A)** Criar uma read replica e gravar sessões diretamente nela.
- **B)** Adicionar ElastiCache for Redis e aplicar cache-aside com TTL, tratamento de cache miss e invalidação coerente.
...
- **B:** Correta — Redis oferece acesso em memória de baixa latência e remove leituras repetitivas do banco; cache-aside mantém o banco como fonte de verdade.
...
## 62. Consolidated billing e compartilhamento de descontos

> **ID:** `SIM01-Q062` · **Domínio:** D4 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A foodtech panela precisa corrigir uma descoberta da revisão Well-Architected. A equipe precisa provar a decisão com um teste pequeno e reversível. Uma empresa possui muitas contas AWS e quer uma fatura consolidada, visão central de custos e melhor aproveitamento agregado de descontos elegíveis. Qual recurso usar? Considere o principal trade-off operacional e escolha a melhor solução.

- **A)** Criar VPC peering entre contas para combinar as faturas.
- **B)** Duplicar os mesmos compromissos em cada conta sem analisar uso agregado.
...
- **D)** Compartilhar a senha do usuário raiz entre todas as equipes.
...
- **A:** Incorreta — Peering conecta redes e não consolida cobrança.
- **B:** Incorreta — Isso pode gerar excesso de compromisso e perde o benefício da visão consolidada.
...
- **D:** Incorreta — Isso é inseguro e não cria organização ou faturamento consolidado.
...
## 63. Security groups e network ACLs

> **ID:** `SIM01-Q063` · **Domínio:** D1 · **Dificuldade:** difícil · **Tempo:** 130 s · **Pontos:** 3

A fintech aurora está separando produção e desenvolvimento em contas distintas. A equipe precisa provar a decisão com um teste pequeno e reversível. Servidores web em subnets públicas aceitam HTTPS do mundo e acessam servidores de aplicação em subnets privadas. A equipe quer controles stateful por recurso e uma camada stateless de bloqueio explícito por CIDR na subnet. Qual combinação usar? Priorize simultaneamente o requisito explícito, o menor acoplamento operacional e o comportamento em falhas.
...
## 64. Fan-out com SNS e SQS

> **ID:** `SIM01-Q064` · **Domínio:** D2 · **Dificuldade:** fácil · **Tempo:** 90 s · **Pontos:** 1

A rede de hotéis brisa precisa atender a uma auditoria sem redesenhar toda a aplicação. Os dados incluem informações reguladas e devem permanecer auditáveis. Cada evento de pedido deve ser processado independentemente por faturamento, estoque e analytics. Se um consumidor parar, os outros devem continuar e o backlog daquele consumidor deve ser preservado. Qual arquitetura é apropriada? Escolha a alternativa que atende diretamente ao requisito.

- **A)** Publicar em uma SNS topic e criar uma fila SQS separada para cada consumidor, com subscriptions e DLQs próprias.
...
- **D)** Guardar eventos em logs locais da instância produtora.
...
- **A:** Correta — SNS replica cada mensagem para as filas assinantes; cada SQS mantém seu próprio backlog, ritmo e política de falhas, isolando consumidores.
...
- **D:** Incorreta — Logs locais não são um canal durável e consumível de integração.
...
## 65. Chave de partição do DynamoDB

> **ID:** `SIM01-Q065` · **Domínio:** D3 · **Dificuldade:** média · **Tempo:** 110 s · **Pontos:** 2

A universidade saber precisa corrigir uma descoberta da revisão Well-Architected. A aplicação é global e a latência percebida pelo usuário é uma métrica de negócio. Uma tabela DynamoDB recebe gravações intensas. A chave de partição atual usa apenas o código de um país, e um país concentra 80% do tráfego, causando throttling. Qual redesign é mais apropriado? Considere o principal trade-off operacional e escolha a melhor solução.
